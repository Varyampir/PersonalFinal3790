...
function locListCtrl($scope, $http) {
  $http.jsonp('http://loc.gov/pictures/collections?fo=json&callback=process=JSON_CALLBACK').success(function(data) {
    ...
    loc_api_images[currentIndex].toptitle = 'Popular Collections';
  ...
  var myTimmer = setInterval(updateImage,7000);
  ...
  function searchInLoc() {

    clearInterval(updateImage);

    console.log("trying to clear the interval and search for " + searchInput);

    $http.jsonp('http://loc.gov/pictures/collections?q=' + searchInput + 'fo=json&callback=process=JSON_CALLBACK').success(function(data) {

      /*Set the $scope.whatever we need (.whatever being the class of the element/tag) to browse through all data.*/
     $scope.searchLoc = data.collections;
    });
  }
}

var searchLocApp = angular.module('searchLocApp',[]);
searchLocApp.controller('locListCtrl', locListCtrl);
